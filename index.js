    ...
    if (req.isAuthenticated()) {
        ...
        if (req.flash("error")) {
            res.render("login", {
                ...
                page: "login",
                name: "Login",
            }); // Pass error message to your login view
        } else {
        ...
        if (req.isAuthenticated()) {
            try {
                ...
                const result = await db.query(
                    "SELECT * FROM userprogress WHERE email = $1 AND level = $2",
                    [req.user.email, parseInt(CurrLevel)]
                );
...
app.post("/api/toggleLights", (req, res) => {
    ...
    req.session.gameEnded = req.session.board.every((row) =>
        row.every((cell) => !cell)
    );
                ...
                if (result.rows.length > 0) {
                    ...
                    if (clickCount < storedMoves) {
                        ...
                        db.query(
                            ...
                            .then(() => {
                                console.log(
                                    `Updated moves for level ${req.session.level} and email ${email}`
                                );
                            })
                            .catch((error) => {
                    ...
                    db.query(
                        ...
                        .then(() => {
                            console.log(
                                `Inserted new progress for level ${req.session.level} and email ${email}`
                            );
                        })
                        .catch((error) => {
...
app.post("/levels", async (req, res) => {
    ...
    <style>
        ...
        if (req.session.level) {
            ...
            return res.status(400).send("Level not specified");
        }
    }
});
...
const create3Grid = (req, matrixSize3, level3) => {
    ...
    const buttonPresses = new Array(matrixSize3)
        .fill()
        .map(() => new Array(matrixSize3).fill(0));
...
app.get("/state3", (req, res) => {
    ...
    // const level = 2;
    req.session.board3 = create3Grid(
        req,
        req.session.matrixSize3,
        req.session.level3
    );
    ...
    if (req.isAuthenticated()) {
        res.render("3state.ejs", {
            board: req.session.board3,
            target: req.session.sumOfValues,
            level: req.session.level3,
            matrixSize: req.session.matrixSize3,
            name: req.user.name,
            profileImage: req.user.profile_image,
        });
    ...
    if (id === "0" && req.session.level3 > 1) {
        ...
        if (req.isAuthenticated()) {
            try {
                // Check if req.session.level3 exists in the database for the authenticated user
                const result = await db.query(
                    "SELECT * FROM userprogress3 WHERE email = $1 AND level = $2",
                    [req.user.email, parseInt(CurrLevel)]
                );
                ...
                if (result.rows.length > 0) {
                    ...
                    const result = await db.query(
                        "SELECT * FROM userprogress3 WHERE email = $1 AND level = (SELECT MAX(level) FROM userprogress3 WHERE email = $1)",
                        [req.user.email]
                    );
...
app.post("/api/toggle3Lights", (req, res) => {
    ...
    req.session.gameEnded = req.session.board3.every((row) =>
        row.every((c) => !c)
    );
                ...
                if (result.rows.length > 0) {
                    ...
                    if (clickCount < storedMoves) {
                        ...
                        db.query(
                            ...
                            .then(() => {
                                console.log(
                                    `Updated moves for level ${req.session.level3} and email ${email}`
                                );
                            })
                            .catch((error) => {
                                console.error("Error updating moves:", error);
                    ...
                    db.query(
                        ...
                        .then(() => {
                            console.log(
                                `Inserted new progress for level ${req.session.level3} and email ${email}`
                            );
                        })
                        .catch((error) => {
                            console.error("Error inserting progress:", error);
...
app.get("/rules", (req, res) => {
    if (req.isAuthenticated()) {
        res.render("rules.ejs", { name: req.user.name });
    } else {
        res.render("rules.ejs", { name: "Login" });
    }
});
            ...
            if (result.rows.length > 0) {
                ...
                transporter.sendMail(mailOptions, (error, info) => {
                    if (error) {
                        ...
                        db.query(
                            "INSERT INTO OTP (email, otp, expiration_time) VALUES ($1, $2, $3)",
    ...
    try {
        ...
        if (enteredOTP === storedOTP) {
            ...
            res.render("login", {
                ...
                password,
                partialEmail: partial,
                message: "OTP entered is wrong! Try again.",
            });
        }
    } catch (error) {
    ...
    db.query("SELECT * FROM Users WHERE email = $1", [email])
                ...
                db.query(
                    ...
                    .catch((error) => {
                        ...
                        res.render("forgetPassword", {
                            page: "enterEmail",
                            email: email,
                            message: "Error storing OTP! Try again.",
                            name: "Login",
                        });
                    });
            }
        ...
        .catch((error) => {
            ...
            res.render("forgetPassword", {
                page: "enterEmail",
                email: email,
                message: "Error checking user existence! Try again.",
                name: "Login",
            });
        });
});
...
app.post("/enter-otp", (req, res) => {
    ...
    db.query(otpQuery)
        ...
        .catch((error) => {
            ...
            res.render("forgetPassword", {
                page: "enterOTP",
                message: "Error fetching OTP! Try again.",
                email: email,
                name: "Login",
            });
        });
                    ...
                    .then(() => {
                        ...
                        db.query(
                            ...
                            .catch((error) => {
                                ...
                                res.render("forgetPassword", {
                                    page: "enterEmail",
                                    message: "Error storing new OTP! Try again.",
                                    name: "Login",
                                });
                            });
                    })
        ...
        const fetchUserData = async () => {
            try {
                ...
                const overallAccuracy =
                    totalMoves > 0
                        ? ((totalTargetMoves / totalMoves) * 100).toFixed(2)
                        : 0;
    ...
    new GoogleStrategy(
        {
            ...
            callbackURL:
                "https://lightsout-w861.onrender.com/auth/google/userProfile",
            userProfileURL: "https://www.googleapis.com/oauth2/v3/userinfo",
        },
        async (accessToken, refreshToken, profile, cb) => {
            try {
                const pictureUrl =
                    profile.photos[0].value && profile.photos[0].value.length > 0
    ...
    new GitHubStrategy(
        {
            ...
            callbackURL:
                "https://lightsout-w861.onrender.com/auth/github/userProfile",
        },
        async (accessToken, refreshToken, profile, cb) => {
            try {
...
function performMatrixOperations5(matrix01) {
    ...
    const modifiedResult1 = resultMatrix1
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult2 = resultMatrix2
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult3 = resultMatrix3
        .toArray()
        .map((row) => row.map((value) => value % 2));
...
function performMatrixOperations4(matrix1) {
    ...
    const resultMatrix11 = add(
        matrix01,
        matrix(matrix02),
        matrix(matrix03),
        matrix(matrix04)
    );
    const resultMatrix12 = add(
        matrix01,
        matrix(matrix03),
        matrix(matrix04),
        matrix(matrix05)
    );
    const resultMatrix13 = add(
        matrix01,
        matrix(matrix04),
        matrix(matrix05),
        matrix(matrix02)
    );
    const resultMatrix14 = add(
        matrix01,
        matrix(matrix05),
        matrix(matrix02),
        matrix(matrix03)
    ...
    const modifiedResult4 = resultMatrix4
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult5 = resultMatrix5
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult6 = resultMatrix6
        .toArray()
        .map((row) => row.map((value) => value % 2));

    const modifiedResult7 = resultMatrix7
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult8 = resultMatrix8
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult9 = resultMatrix9
        .toArray()
        .map((row) => row.map((value) => value % 2));

    const modifiedResult10 = resultMatrix10
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult11 = resultMatrix11
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult12 = resultMatrix12
        .toArray()
        .map((row) => row.map((value) => value % 2));

    const modifiedResult13 = resultMatrix13
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult14 = resultMatrix14
        .toArray()
        .map((row) => row.map((value) => value % 2));
    const modifiedResult15 = resultMatrix15
        .toArray()
        .map((row) => row.map((value) => value % 2));
